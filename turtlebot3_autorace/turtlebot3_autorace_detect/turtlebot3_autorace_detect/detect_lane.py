...
class DetectLane(Node):
    ...
    def cbFindLane(self, image_msg):
        ...
        '''추가 - 이미지를 전처리 하는 함수'''
        # cv_image = self.preprocessing_image(cv_image)
    ...
    def make_lane(self, cv_image, white_fraction, yellow_fraction):
        ...
        if yellow_fraction > 3000:
            ...
            cv2.polylines(cv_image, np.int_([pts_left]), isClosed=False, color=(0, 0, 255), thickness=25)   ##### 추가
            # cv2.polylines(color_warp_lines, np.int_([pts_left]), isClosed=False, color=(0, 0, 255), thickness=25)

        if white_fraction > 3000:
            pts_right = np.array([np.transpose(np.vstack([self.right_fitx, ploty]))])
            cv2.polylines(cv_image, np.int_([pts_right]), isClosed=False, color=(255, 255, 0), thickness=25)    ##### 추가
            # cv2.polylines(color_warp_lines, np.int_([pts_right]), isClosed=False, color=(255, 255, 0), thickness=25)
        
        if white_fraction > 3000 and yellow_fraction > 3000:             ##### 추가
            centerx = np.mean([self.left_fitx, self.right_fitx], axis=0) ##### 추가
        ...
        if centerx is not None:
            ...
            cv2.polylines(cv_image, np.int_([pts_center]), isClosed=False, color=(0, 255, 255), thickness=12)   ##### 추가
        ...
        # x축 500에 중심선 그리기
        cv2.line(cv_image, (500, 0), (500, cv_image.shape[0]), (0, 255, 0), 4)  ##### 추가
