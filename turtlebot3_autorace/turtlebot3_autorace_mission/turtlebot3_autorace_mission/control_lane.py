...
class ControlLane(Node):
    ...
    def callback_follow_lane(self, desired_center):
        ...
        if self.state == 'drive':
            ...
            twist.linear.x = min(self.MAX_VEL * (max(1 - abs(error) / 500, 0) ** 2.2), 0.07)
            twist.angular.z = -max(angular_z, -1.5) if angular_z < 0 else -min(angular_z, 1.5)
    ...
    def callback_robot_state(self, state_msg):
